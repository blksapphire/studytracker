    ...
    LazyColumn(contentPadding = PaddingValues(18.dp, 4.dp, 18.dp, 30.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                Box(Modifier.fillMaxWidth().background(Brush.linearGradient(listOf(TealPrimary, Color(0xFF817BFF), SkyAccent)), RoundedCornerShape(30.dp)).padding(22.dp)) {
                    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(9.dp)) {
                            Pill("Level ${state.level}", Icons.Filled.Star)
                            Pill("${state.xp} XP", Icons.Filled.AutoAwesome)
                        }
                    }
                }
            }
        ...
        item {
            ElevatedCard(shape = RoundedCornerShape(24.dp)) {
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("Level ${state.level}", fontWeight = FontWeight.Bold); Text("${ceil((250 - (state.xp - levelStart).coerceIn(0, 249)).toDouble()).toInt()} XP to next", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant) }
                    LinearProgressIndicator({ levelProgress }, Modifier.fillMaxWidth().height(8.dp), strokeCap = androidx.compose.ui.graphics.StrokeCap.Round)
        ...
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
                StatCard(Modifier.weight(1f), Icons.Filled.Schedule, formatDuration(state.totalStudyMillis), "Study time")
                StatCard(Modifier.weight(1f), Icons.Filled.LocalFireDepartment, "${state.streakDays}d", "Current streak")
            }
        }
        item {
            Row(horizontalArrangement = Arrangement.spacedBy(10.dp), modifier = Modifier.fillMaxWidth()) {
                StatCard(Modifier.weight(1f), Icons.Filled.MenuBook, state.sessions.toString(), "Sessions")
                StatCard(Modifier.weight(1f), Icons.Filled.LocalFireDepartment, "${state.bestStreakDays}d", "Best streak")
            }
        }
        item {
            ElevatedCard(shape = RoundedCornerShape(24.dp)) {
                Column(Modifier.padding(18.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    ...
                    if (state.examDateMillis == null) Text("No exam date yet. Add one in Goals to make the countdown work for you.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                        ...
                        Text(SimpleDateFormat("EEEE, d MMMM yyyy", Locale.getDefault()).format(Date(state.examDateMillis)), style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                }
            }
        }
        if (state.subjects.isNotEmpty()) {
            ...
            item {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
                    state.subjects.take(4).forEach { SuggestionChip(onClick = {}, label = { Text(it) }) }
                }
            }
        }
    }
...
private fun Pill(text: String, icon: androidx.compose.ui.graphics.vector.ImageVector) {
    Surface(color = Color.White.copy(alpha = .15f), shape = RoundedCornerShape(50.dp)) { Row(Modifier.padding(horizontal = 11.dp, vertical = 7.dp), verticalAlignment = Alignment.CenterVertically) { Icon(icon, null, tint = Color.White, modifier = Modifier.size(15.dp)); Spacer(Modifier.width(5.dp)); Text(text, color = Color.White, style = MaterialTheme.typography.labelMedium, fontWeight = FontWeight.Bold) } }
}

@Composable
private fun StatCard(modifier: Modifier, icon: androidx.compose.ui.graphics.vector.ImageVector, value: String, label: String) {
    ElevatedCard(modifier = modifier, shape = RoundedCornerShape(22.dp)) { Column(Modifier.padding(15.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) { Icon(icon, null, tint = TealPrimary, modifier = Modifier.size(21.dp)); Text(value, style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Bold); Text(label, style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.onSurfaceVariant) } }
}
